...
contract LeaseAgreement is Ownable, ERC721Enumerable {
  ...
  event LogNewInvestor(address indexed _address, block.timestamp)
  event RemovedInvestor (address indexed _address, block.timestamp)
  ...
  function addInvestor (address _address) external onlyOwner {
    for (uint256 i = 0; i < investors.length; i++) {
      ...
      Investor[_address].index = invested.push(_address) - 1;
    }
  }
